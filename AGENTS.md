# go-boot-jwt 项目开发规范文档

go-boot-jwt 是一个基于 [github.com/xudefa/go-boot](https://github.com/xudefa/go-boot) 的 JWT（JSON Web Token）认证集成模块。本模块提供令牌生成、解析、验证和刷新功能，并内置 JWT 认证过滤器，遵循 go-boot 项目的开发规范。
...
- **集成模块**：go-boot-jwt 是 go-boot 的安全层集成，将 JWT 作为认证机制
- **规范继承**：完全遵循 go-boot 的开发规范、命名约定、代码风格

### 1.2 核心职责

- 将 JwtUtil 注册为 go-boot 容器中的 Bean
- 提供基于 go-boot 自动配置的 JWT 启动器
- 实现 `SecurityFilter` 接口的 JWT 认证过滤器
- 支持令牌生成、解析、验证和刷新的声明式操作
...
- **JWT 库**：集成 golang-jwt/jwt/v5（`github.com/golang-jwt/jwt/v5`）
- **职责边界**：仅负责 JWT 集成，不包含其他业务逻辑
- **示例代码**：统一放在 `examples/` 目录，演示 JWT 集成用法

### 2.2 go-boot-jwt 核心包结构

| 包 | 说明 | 接口定义 |
|---|------|----------|
| `jwt/` | JWT 工具类（令牌操作、认证过滤器） | `JwtUtil`, `JwtAuthenticationFilter` |

### 2.3 go-boot 核心包参考

go-boot-jwt 依赖 go-boot 的以下核心包：
...
go-boot-jwt 遵循 go-boot 的接口抽象原则，所有集成层通过核心框架中的接口抽象定义，实现运行时互换：
